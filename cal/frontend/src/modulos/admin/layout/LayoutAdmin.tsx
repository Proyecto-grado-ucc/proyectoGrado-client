...
import { LayoutDashboard, Calendar, ClipboardList, Users, Settings, Activity, Menu, X, LogOut, GraduationCap, Moon, Sun } from 'lucide-react';
import { useAuthStore } from '../../../seguridad/store';

const menu = [
  { path: '/admin/dashboard', icono: LayoutDashboard, label: 'Dashboard' },
  { path: '/admin/horarios', icono: Calendar, label: 'Horarios' },
  { path: '/admin/evaluacion', icono: ClipboardList, label: 'Evaluación' },
  { path: '/admin/usuarios', icono: Users, label: 'Usuarios' },
  { path: '/admin/configuracion', icono: Settings, label: 'Configuración' },
  { path: '/admin/auditoria', icono: Activity, label: 'Auditoría' },
];
...
export default function LayoutAdmin() {
  ...
  const iniciales = usuario?.email?.substring(0, 2).toUpperCase() ?? 'AD';
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900 flex-col md:flex-row transition-colors duration-300">
      ...
      <div className="md:hidden flex items-center justify-between bg-blue-950 dark:bg-gray-950 p-4 text-white shrink-0 border-b border-blue-900 dark:border-gray-800 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-blue-300 text-[10px] font-medium uppercase tracking-widest">Academy</p>
          </div>
        </div>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 lg:w-56 bg-blue-950 dark:bg-gray-950 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 flex flex-col shadow-xl md:shadow-none ${menuAbierto ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between px-6 py-6 border-b border-white/5 md:justify-start">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-blue-300 text-[11px] font-medium uppercase tracking-widest">Academy</p>
            </div>
          </div>
          ...
          {menu.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  isActive
                    ? 'flex items-center gap-3 px-4 py-3 rounded-xl text-sm bg-blue-600/20 text-blue-400 font-medium border border-blue-500/20 transition-all'
        ...
        <div className="px-6 py-6 border-t border-white/5 bg-black/10">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full bg-blue-600/20 border border-blue-500/30 flex items-center justify-center text-blue-400 text-sm font-bold shrink-0">
              {iniciales}
            </div>
            <div className="truncate">
              <p className="text-white text-sm font-medium truncate">{usuario?.email?.split('@')[0] || 'Administrador'}</p>
              <p className="text-blue-400 text-xs font-medium uppercase tracking-wider mt-0.5">Admin</p>
            </div>
          </div>
          <button
            onClick={manejarSalida}
            className="w-full flex items-center justify-center gap-2 text-sm font-medium text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 py-2.5 rounded-xl transition-all border border-white/5 hover:border-white/10"
          >
      ...
      <main className="flex-1 overflow-auto bg-gray-50 dark:bg-gray-900 h-[calc(100vh-72px)] md:h-screen w-full relative transition-colors duration-300">
        <Outlet />
      </main>
    </div>
  );
