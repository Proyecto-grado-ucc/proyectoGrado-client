...
const menu = [
  { path: '/admin/dashboard', icono: 'D', label: 'Dashboard' },
  { path: '/admin/horarios', icono: 'H', label: 'Horarios' },
  { path: '/admin/evaluacion', icono: 'E', label: 'Evaluacion' },
  { path: '/admin/usuarios', icono: 'U', label: 'Usuarios' },
  { path: '/admin/configuracion', icono: 'C', label: 'Configuracion' },
  { path: '/admin/auditoria', icono: 'A', label: 'Auditoria' },
];
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 md:w-48 bg-blue-950 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 flex flex-col ${menuAbierto ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="px-4 py-4 border-t border-white/10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              <p className="text-white text-xs font-medium">Juan Burbano</p>
              <p className="text-blue-300 text-xs">Administrador</p>
            </div>
          </div>
          <button
            onClick={manejarSalida}
            className="w-full text-xs text-blue-300 hover:text-white hover:bg-white/10 py-1 rounded transition-colors"
          >
            Cerrar sesion
          </button>
        </div>
